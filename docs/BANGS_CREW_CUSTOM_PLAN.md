# Bang's Crew Custom Plan

주원 전용 확장형 에이전트 대시보드 커스터마이징 계획

## 목표
- 현재/미래 에이전트 운영 현황을 한 화면에서 확인 (인원 증가 자동 반영)
- 크론 건강도/실패 원인/다음 실행 시간 즉시 파악
- 방울이의 케어 활동(점검/조치 로그)을 시각화

## 1차 적용(이번 커밋)
- 기본 브랜딩 텍스트를 `Bang's Crew Mission Control`로 변경
- 랜딩/푸터에 패밀리 운영 목적 문구 반영

## 2차 예정
1. Agent Cards 동적 생성
   - agents.list 기반 자동 카드(상태/최근 활동/담당업무)
2. Cron Timeline
